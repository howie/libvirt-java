...
public final class TestJavaBindings extends TestCase {
	final int UUIDArray[] = {	Integer.decode("0x00"),
								Integer.decode("0x4b"),
								Integer.decode("0x96"),
								Integer.decode("0xe1"),
								Integer.decode("0x2d"),
	...
	public void testStoragePool() throws Exception {
		StoragePool pool1 = conn.storagePoolDefineXML("<pool type='dir'>" + "  <name>pool1</name>" + "  <target>"
				+ "    <path>/pool1</path>" + "  </target>" + "  <uuid>004c96e1-2d78-c30f-5aa5-f03c87d21e67</uuid>"
				+ "</pool>", 0);
	...
	public void testDomainEvents() throws Exception {
		...
		try {
			...
			assertEquals(Arrays.asList(	Connect.DomainEvent.LifecycleCallback.Event.DEFINED,
										...
										Connect.DomainEvent.LifecycleCallback.Event.UNDEFINED), events);
		}
		finally {
			if(cbId != -1)
				conn.domainEventDeregister(cbId);
			this.stop = true;
		}
